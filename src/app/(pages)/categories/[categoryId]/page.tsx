...
import { getCateoryData } from '../_action/getCaregory.action';

export default async function CategoryContent({params}:{params:Params}) {
  let {categoryId}= await params;
   
    const {data:products} :{data:product[]}=await getCateoryData(categoryId as string);
...
const content =await res.json()
  return (
    <>
    <h3 className="text-4xl font-bold text-center text-gray-800 pb-5 ">{content.data.name}</h3>
    {
       products.length>0? <div className='grid grid-cols-1 px-4 md:px-0 gap-4 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4'>
